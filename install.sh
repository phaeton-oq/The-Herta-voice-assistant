...
# `|| code=$?` обязателен: при set -e падение установщика оборвало бы скрипт
# прямо здесь, и следующая строка с чтением кода уже не выполнилась бы.
code=0
"$venv/bin/python" "$root/tools/installer.py" "$@" || code=$?
