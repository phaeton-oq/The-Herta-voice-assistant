...
- `keyring` для хранения ключей API в системном хранилище учётных данных
...
Установщик сам создаёт окружение, подбирает сборку `torch` под твою видеокарту, ставит зависимости, проверяет внешние программы и в конце прогоняет самодиагностику.
...
./install.sh
```
Из `git clone` файл уже исполняемый. Если скачивал архивом — сначала `chmod +x install.sh`.

Он спросит, что ставить:

| Тир | Что входит | Сколько весит |
|---|---|---|
| **Базовый** | текст, графическая оболочка, Telegram-мост, управление компьютером, mypy/ruff | сотни мегабайт, пара минут |
| **С голосом** | плюс `torch`, распознавание речи и синтез | несколько гигабайт |

Можно и без вопросов:

```powershell
.\install.ps1 --tier voice                  # с голосом, сборка torch определится сама
.\install.ps1 --tier voice --torch cu121    # если автоопределение промахнулось
.\install.ps1 --tier base --dry-run         # примерка: показать план, ничего не ставя
```

Про `--torch`: сборка выбирается по версии CUDA, которую сообщает драйвер. Если версию разобрать не вышло, а карта NVIDIA есть, берётся самое свежее колесо — ошибиться в эту сторону дешевле. Неподходящая сборка сразу скажет о себе внятной ошибкой, а вот тихо приехавшая CPU-версия просто делает всё медленным, и догадаться об этом трудно.

Установщик не трогает существующий `.env` и не пересоздаёт рабочее окружение, если оно уже есть.

### Что придётся доставить руками

Четыре вещи pip поставить не может. Установщик проверит их наличие и напечатает точные команды для твоей системы, но саму установку надо запустить самому. Менеджер пакетов он определяет сам — понимает `apt`, `dnf`, `pacman` и `zypper`, а на Windows использует `winget`. Если дистрибутив незнакомый, даст ссылку, а не выдуманную команду.

**Ollama** — локальные модели и зрение.
```powershell
winget install Ollama.Ollama          # Windows
curl -fsSL https://ollama.com/install.sh | sh    # Linux
```
Потом хотя бы одну модель: `ollama pull qwen2.5:7b`, а для зрения — `ollama pull qwen2.5vl:3b`. Если используешь только облачного провайдера, Ollama не нужна вовсе.

**ffmpeg** — голосовые сообщения в Telegram.
```powershell
winget install Gyan.FFmpeg            # Windows
sudo apt install ffmpeg               # Debian/Ubuntu
```
Если у тебя стоит Applio, отдельный ffmpeg не нужен: в его поставке уже есть свой, и установщик это учитывает.

**git** — не обязателен, но с ним удобнее обновляться, и он нужен для команд `git` в терминале Герты.

**Applio/RVC** — собственный голос Герты. Это отдельное приложение со своим Python-окружением и файлом модели, автоматизировать его честно не выйдет. Подробности в разделе «Опционально: RVC-голос Герты» ниже. Без него Герта говорит базовым голосом Silero или через Edge TTS.

**Ключи API** вписываются в `.env` — установщик создаст его из `.env.example`, все настройки там прокомментированы. Минимум для облачного режима: `LLM_PROVIDER` и ключ выбранного провайдера.

### Если установщик не подошёл

Ниже — подробные пошаговые инструкции для обеих систем. Они делают то же самое руками и пригодятся, если что-то пошло не так или хочется понимать каждый шаг.
...
Кнопка **НАСТРОЙКИ** в заголовке окна открывает выбор режима и ключи — редактировать `.env` руками не обязательно.

#### Ключи API прямо в окне

Ключи вводятся в окне настроек и уходят в системное хранилище учётных данных: Credential Manager на Windows, Secret Service на Linux. В файле проекта они при этом не появляются, что важно, раз репозиторий публичный.

Поддерживаются ключи Cerebras, Google AI Studio, DeepSeek/OpenRouter, **Tavily для веб-поиска** и токен Telegram-бота. У каждого поля видно, задан ли ключ и откуда он взят.

Порядок такой: сначала системное хранилище, потом `.env`. Побеждает то, что задано последним через окно — иначе получалось бы, что ключ сохранил, а работает старый из файла, и понять это невозможно. Если нужно вернуть управление файлу, ключ из хранилища удаляется кнопкой рядом.

Если `keyring` не установлен или в системе нет подходящего хранилища (обычное дело на сервере без графической сессии), окно честно об этом скажет, и ключи будут читаться только из `.env`.

#### Переключение локально / облако

В том же окне выбирается провайдер и модель, после чего **Применить режим** переключает Герту без перезапуска и без потери разговора.

Что происходит при переключении: пересобирается не только клиент, но и весь системный префикс. Persona-слой зависит от модели — слабым моделям достаётся компактная версия. А инструкция об инструментах зависит от провайдера: Cerebras и Ollama не умеют structured tool calling, и если им об этом не сказать, модель начинает печатать JSON вызова прямо в текст ответа. История диалога при этом сохраняется целиком.

При уходе с Ollama на облако локальная модель выгружается из видеопамяти — на 6 ГБ она иначе продолжает занимать место рядом с Whisper и RVC.

Переключение во время ответа не сработает: Герта попросит дождаться конца реплики.
