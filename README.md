...
Локальный голосовой ассистент в образе Великой Герты из Honkai: Star Rail. **Текущая версия: v0.6.**
...
Новое в v0.6:
- **навыки** (`skills/`): подробные инструкции подмешиваются только когда разговор доходит до дела. В постоянном промпте живёт список из трёх строк, полный текст приходит на один ход. В комплекте `code`, `study` и `research`; свой навык добавляется папкой с файлом, без правки кода
- **консольная Герта** (`herta_cli.py`): третья точка входа, команды через слэш, разметка ответов через `rich`
- **панель владельца в Telegram**: `/admin` показывает провайдера, активные навыки, кто пишет и сколько; `/skills` переключает навыки прямо из чата
- **переключатель навыков в окне**: НАСТРОЙКИ → Навыки

Новое в v0.5:
- **установщик** (`install.ps1`, `install.sh`): два тира, автоподбор сборки torch под видеокарту, проверка внешних программ, самодиагностика в конце
- **ключи API прямо в окне**: уходят в системное хранилище учётных данных, а не в файл проекта
- **переключение локально/облако без перезапуска**: провайдер и модель меняются из окна, разговор сохраняется
...
Иначе говоря: к v0.6 у ассистента есть лицо, руки, три канала связи и разные режимы работы. Он живёт в окне, приходит по хоткею из любого приложения, отвечает в терминале и в Telegram, видит экран, работает с редактором, составляет мнение о собеседниках и меняет манеру в зависимости от того, о чём разговор. Произвольного доступа к системе у него по-прежнему нет — только перечисленные обратимые действия и белый список команд с подтверждением.
...
- `rich` для терминальной оболочки с разметкой ответов
...
├─ herta_cli.py        # терминальная Герта с командами через слэш
...
├─ config.py
├─ skills/             # навыки: по папке с SKILL.md на каждый
│  ├─ code/
│  ├─ study/
│  └─ research/
├─ audio/
├─ actions/            # инструменты: поиск, mypy/ruff, зрение, системные действия
├─ brain/              # память, факты, профили людей, впечатления, навыки
├─ bridges/            # Telegram: мост, разметка, голосовые, хранилище
├─ desktop/            # хоткеи, диктовка, управление ПК, терминал, редактор
├─ gui/                # окно, аватар, панели, настройки, воркеры
├─ tools/              # установщик и вспомогательные скрипты
├─ stt/
├─ tts/
├─ llm/
├─ persona/
├─ utils/              # логи, оценка токенов, хранилище ключей
└─ wakeword/
```
...
Инструкции, которые подмешиваются только когда разговор доходит до дела.

### Зачем

Системный промпт Герты занимает около семи тысяч токенов и грузится всегда — даже когда речь о погоде. Каждый новый набор умений раньше означал ещё кусок текста в этой постоянной части и ещё десяток правил разбора в общем файле. На двадцати с лишним инструментах подход упёрся: правила начинают пересекаться и ломать друг друга.

Навык собирает описание, слова-приметы и подробные указания в одном месте. В постоянном промпте живёт **только строка описания** на навык — 111 токенов на все три. Полный текст, 1905 токенов, приходит на один ход.

### Что в комплекте

| Навык | О чём |
|---|---|
| `code` | разбор кода, проверка типов и стиля, тесты, переход к находке |
| `study` | вести к пониманию, а не выдавать ответ: найти границу знания, дать один шаг, проверить усвоение |
| `research` | разобраться в незнакомой теме: честно отделить знание от незнания, сверить источники, назвать расхождения |

### Как выбирается навык

Подбор локальный, по приметам. Приметы двух уровней: обычная даёт балл, безошибочная — сразу проходной. Порог срабатывания — два балла.

```
triggers: [код, функци, тест, ошибк]        # по баллу каждая
strong:   [mypy, ruff, pytest, traceback]   # каждой хватает одной
```

Разделение появилось после разбора настоящих переписок. Первая версия включала навык по одному совпадению и на шестидесяти реальных репликах дала четыре ложных срабатывания из шести: «тест» внутри «тестирования UI/UX», «почему» в личном вопросе, «исследован» в метафоре. С порогом осталось одно пограничное, а 98% обычного разговора идёт вообще без навыка.

К модели обращаемся только при настоящем сомнении — когда несколько навыков набрали поровну. На «привет» роутинг не тратится: платить лишним запросом за каждую реплику бессмысленно.

Подбор словарный, и это его потолок. Он покрывает перечисленные приметы, а не произвольные формулировки.

### Включать и выключать

- **в окне** — НАСТРОЙКИ → Навыки, галочка на каждый
- **в консоли** — `/skills on|off <имя>`
- **в Telegram** — `/skills off study`, только владельцу

Выключенный навык остаётся на диске и возвращается одним щелчком. Решение хранится в `data/skills_state.json` и переживает перезапуск.

### Свой навык

Создай папку `skills/<имя>/` с файлом `SKILL.md`:

```markdown
---
name: music
title: Музыка
description: разбор треков, теория, подбор на слух
triggers: [аккорд, тональность, ритм, гармони]
strong: [подобрать на слух, теория музыки]
tools: []
---

Дальше идут инструкции, которые Герта получит при срабатывании навыка.
Пиши их как указания ей, а не как описание для читателя.
```

Обязательны только `name`, `title` и `description`. Битый файл не лишает Герту остальных навыков: ошибка попадёт в лог с именем файла и номером строки.

Файл читается как `utf-8-sig` — Блокнот и PowerShell дописывают в начало BOM, и без этого заголовок переставал распознаваться.
...
| `python herta_cli.py` | терминальная Герта с командами через слэш |
| `python main.py` | консольный режим (текст, `--voice`, `--live-voice`) |
| `python telegram_app.py` | Telegram-мост |

Все команды одним списком лежат в [`commands.txt`](commands.txt).
...
Тот же мозг, что в окне и в Telegram, но разговор в терминале. Разметку ответов рисует `rich`, поэтому код, таблицы и списки видно нормально, а не сплошным текстом со звёздочками.

Всё, что не начинается со слэша, уходит Герте как реплика. Команды:

| Команда | Что делает |
|---|---|
| `/help` | список команд |
| `/skills` | навыки: ● включён, ○ выключен |
| `/skills off study` | переключить навык |
| `/model` | показать провайдера и модель |
| `/model ollama qwen2.5:7b` | сменить без перезапуска, разговор сохраняется |
| `/memory` | что Герта помнит о тебе |
| `/context` | сколько занято контекста |
| `/clear` | забыть разговор, долговременную память не трогает |
...
Команды бота: `/help`, `/whoami`, `/reset`, `/voice`, `/impression`, `/forgetme`.

Только владельцу:

| Команда | Что показывает |
|---|---|
| `/admin` | панель: провайдер и модель, активные навыки, зрение и веб-поиск, режим голосовых, кто пишет и сколько, сколько профилей с мнением |
| `/skills` | список навыков; `/skills off study` переключает |
| `/people` | список известных людей |

Переключение навыка сбрасывает активные сессии, иначе изменение дошло бы только до новых собеседников. Гостям переключатель не даём: выключив себе `research`, человек потом решит, что Герта сломалась.
...
SKILLS_ENABLED=true
SKILLS_DIR=skills
SKILLS_ASK_MODEL_WHEN_UNSURE=true
...
- подбор навыка тоже словарный: на шестидесяти реальных репликах остаётся одно пограничное срабатывание из шестидесяти
- навыки `study` и `research` упоминают инструменты памяти, которых у гостей в Telegram нет — инструкция обещает то, чего они не смогут вызвать
- панель владельца в Telegram текстовая: кнопок нет, для них мосту нужно принимать `callback_query`
- локальная модель и голосовой режим вместе в 6 ГБ VRAM не помещаются: при `OLLAMA_NUM_CTX=8192` модель занимает ~4.2 ГБ, и на Whisper места уже не остаётся
